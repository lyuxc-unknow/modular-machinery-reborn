  ...
  public <T> DataResult<Map<K, V>> decode(DynamicOps<T> ops, MapLike<T> input) {
    ...
    if(input.get("default") != null) {
      ...
      if(defaultResult.result().isPresent())
        ...
        MMRLogger.INSTANCE.warn("Couldn't parse value for key {} in MapLike {}, {}", "default", input.toString(), defaultResult.error().get().message());
    }
    ...
    input.entries().forEach(entry -> {
      ...
      if(keyResult.result().isPresent()) {
        ...
        if(valueResult.result().isPresent())
          ...
          MMRLogger.INSTANCE.warn("Couldn't parse value for key {} in MapLike {}, {}", key.toString(), input.toString(), valueResult.error().get().message());
      }
    });
