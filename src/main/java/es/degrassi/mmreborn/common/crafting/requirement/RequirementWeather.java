...
public class RequirementWeather extends ComponentRequirement<RequirementWeather.WeatherType, RequirementWeather> {
  ...
  public @NotNull CraftCheck canStartCrafting(ProcessingComponent<?> component, RecipeCraftingContext context,
    ...
    if (switch (weather) {
      ...
      case SNOW ->
          world.isRaining() && world.canSeeSky(pos.above()) && world.getHeightmapPos(Heightmap.Types.MOTION_BLOCKING, pos.above()).getY() > pos.above().getY() && world.getBiome(pos).value().coldEnoughToSnow(pos.above());
